...
} color_ids = { .red        = 789253280374849636,
...
#define match_role(x, y) \
    if (!strcmp (x, response)) return color_ids.y;

u64snowflake color_role_id (const char *response) {
    match_role ("red", red);
...
void color_command_interaction (struct discord *handle,
    ...
    if (event->data->options->size != 1)
        ...
        u64snowflake role = color_role_id (event->data->options->array->value);
        ...
        if (!role) strcpy (response, "invalid role specified");
            ...
            for (int i = 0;
                ...
                u64snowflake target = ((u64snowflake *) &color_ids)[i];

                if (target != role)
                    discord_remove_guild_member_role (
                       handle, event->guild_id, event->member->user->id,
                       ((u64snowflake *) &color_ids)[i], &remove_params, NULL);
            }
